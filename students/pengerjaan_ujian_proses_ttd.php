...
if (!empty($_SESSION['signature'])) {
    $signatureBase64 = str_replace('data:image/png;base64,', '', $_SESSION['signature']);
    $signatureBinary = base64_decode($signatureBase64);
    $signatureBinary = pg_escape_bytea($signatureBinary);
...
try {
    ...
    if($isSubmitFinal){
        $stmt_update = $db->prepare("
            ...
            SET st_selesai = 'Y', signature = ?
            WHERE id_jawaban_siswa = ?
        ");

        $stmt_update->execute([$signatureBinary, $id_jawaban_siswa]);
    }
