...
foreach ($id_subkelas as $subkelas) {
    ...
    //$cekdobel = $bariscekdobel['id_ujian'].$bariscekdobel['id_kelas'].$bariscekdobel['id_subkelas'].$bariscekdobel['kode_mata_pelajaran'].$bariscekdobel['nama_bab'];

    //$kode_unik = $id_ujian.$id_kelas.$subkelas.$kode_mata_pelajaran.$nama_bab;

    if ($bariscekdobel['cek_ada'] > 0) {
    ...
    // Simpan header ujian
    $stmt = $db->prepare("INSERT INTO f_soal_hdr (st_susulan, tanggal_ujian, kode_mata_pelajaran, kode_guru, uuidguru, id_kelas, kode_ujian, durasi, nama_bab, id_ujian, id_subkelas, createuser, token, soal_acak, option_acak) 
                        VALUES (:st_susulan, :tanggal_ujian, :kode_mata_pelajaran, :kode_guru, :uuidguru, :id_kelas, :kode_ujian, :durasi, :nama_bab, :id_ujian, :subkelas, :user_create, :token, :soal_acak, :option_acak)");
    $stmt->execute([
