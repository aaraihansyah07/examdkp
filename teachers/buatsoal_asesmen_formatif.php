...
<!-- Topbar -->
                ...
                <div class="container-fluid">
                    ...
                    <h1 class="h3 mb-2 text-gray-800 ml-1">Buat Soal Asesmen Diagnostik</h1>
                            ...
                            <?php
                                $sql12 = "SELECT DISTINCT(exid) exid from rubrik_asesmen where exid = $exid AND status_rubrik = 'Y'";
                                $hasil12 = mysqli_query($koneksi, $sql12);
                                $baris12 = mysqli_fetch_array($hasil12);

                                if (empty($baris12['exid'])) {
                            ...
                            <a data-toggle="modal" class="btn btn-secondary btn-icon-split" data-target="#createModal">
                                ...
                                <span class="text">Buat Rubrik</span>
                             </a>
                             <?php
                                }
                             ?>
                             <button id="assign" name="assign" class="btn btn-primary btn-user">Assign</button>
                             <button style="background:red; border:1px solid red" id="batal_assign" name="batal_assign" class="btn btn-primary btn-user">Batal Assign</button>
                            </form>
                    <!-- DataTales Example -->
                                <?php
                                    include('config.php');
                                ...
                                <div class="card-body">
                                        ...
                                        <?php  
                                                ...
                                                if (isset($_POST['assign'])) {
                                                        $sql13 = "SELECT DISTINCT(exid) exid from rubrik_asesmen where exid = $exid AND status_rubrik = 'Y'";
                                                        $hasil13 = mysqli_query($koneksi, $sql13);
                                                        $baris13 = mysqli_fetch_array($hasil13);

                                                        if (empty($baris13['exid'])) {
                                                            echo "<script>window.alert('Harap buat rubrik penilaian terlebih dahulu')</script>";
                                                        }
                                                        else {
...
<span>Proudly made by aa_raihansyah</span>
    ...
    <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Buat Rubrik Asesmen Diagnostik</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form class="user" action="buat_jml_kategori_rubrik.php" method="post" style="padding:1% 3% 0 3%">
                    <p><b>Jumlah Kategori Penilaian</b></p>
                    <?php
                        $sql11 = "SELECT kode_assign from asesmen_diag where exid = $exid";
                        $hasil11 = mysqli_query($koneksi, $sql11);
                        $baris11 = mysqli_fetch_array($hasil11);
                        $kode_assign = $baris11['kode_assign'];
                    ?>
                    <div class="form-group">
                        <input type="hidden" name="exid" value="<?php echo $exid;?>"/>
                        <input type="hidden" name="kode_assign" value="<?php echo $kode_assign;?>"/>
                        <select name="level">
                            <option value="">--Pilih Jumlah Kategori--</option>
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancel</button>
                        <button class="btn btn-primary" name="buat_rubrik" href="buat_data_master_kelas.php">Buat</button>
                    </div>
                </form>
            </div>
        </div>
