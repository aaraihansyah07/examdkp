...
<?php
    include('config.php');
    $id_materi = $_GET['id'];
    $sql2 = "SELECT id_kelas, id_guru, nama_materi, id_mata_pelajaran from materi_tugas where id = $id_materi";
    $hasil2 = mysqli_query($koneksi, $sql2);
    $baris2 = mysqli_fetch_array($hasil2);

    $id_kelas = $baris2['id_kelas'];
    $id_guru = $baris2['id_guru'];
    $id_mapel = $baris2['id_mata_pelajaran'];
    $nama_materi = $baris2['nama_materi'];
?>

<!-- Page Heading -->
<h1 class="h3 mb-2 text-gray-800"><?php echo $nama_materi;?></h1>
        <a href="materi_tugas.php" class="btn btn-secondary btn-icon-split">
...
<!-- DataTales Example -->
<?php
    $sql3 = "SELECT isi_materi from detail_soal_formatif_pbl where id_materi = $id_materi order by seq_rubrik_asesmen asc";
    $hasil3 = mysqli_query($koneksi, $sql3);
    
    while($baris3 = mysqli_fetch_array($hasil3)) {
        echo "<div class='card shadow mb-4' style='padding:2%'>";
            echo $baris3['isi_materi'];
        echo "</div>";
    }
?>
</div>
