...
foreach ($soalList as $index => $soal) {
    ...
    if (isset($_FILES['soal']['name'][$index]['gambar']) && $_FILES['soal']['name'][$index]['gambar'] != '') {
        ...
        if ($fileSize > 1024 * 1024) {
            echo "<script>alert('Ukuran gambar soal ke-".($index+1)." melebihi 1MB!'); window.location.href='list_ujian_edit.php?hdrujn=$id_ujian_hdr&mp=".$mp."&kls=".$kls."&ju=".$ju."';</script>";
            exit;
        }
...
if (!empty($warning)) {
    $msg = implode("\\n", $warning);
    echo "<script>alert('Perhatian:\\n$msg'); window.location.href='list_ujian_edit.php?hdrujn=$id_ujian_hdr&mp=".$mp."&kls=".$kls."&ju=".$ju."';</script>";
} else {
    echo "<script>alert('Soal berhasil diperbarui!'); window.location.href='list_ujian_edit.php?hdrujn=$id_ujian_hdr&mp=".$mp."&kls=".$kls."&ju=".$ju."';</script>";
}
?>
