<?php
    ...
    $sqlhdr = "SELECT soal_acak, option_acak, tanggal_ujian, id_ujian, uuidguru, kode_guru, h.kode_mata_pelajaran, kode_ujian, id_kelas, id_subkelas, st_posting, waktu_mulai,
    waktu_berakhir, nama_bab, mp.nama_mata_pelajaran, h.durasi, h.st_susulan, h.st_nonaktif_ujian, h.token
    from f_soal_hdr h
...
<head>
    ...
    <style>
        #icon_list_ujian {color:white}
        #list_ujian {color:white; font-weight:bold}
    </style>
</head>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav style="margin-top:-2.5%; background:#3d4857; height:35px" class="navbar navbar-expand navbar-light topbar mb-2 static-top shadow">
                    <a href="list_ujian.php?mp=<?php echo $mp;?>&kls=<?php echo $kls;?>&ju=<?php echo $ju;?>" style="color:white; margin-left:0.8%; font-size:90%">Daftar Ujian Saya > </a>
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-2">
                        <!-- Tombol Tambah -->
                        <a class="btn btn-secondary btn-icon-split" href="list_ujian.php?mp=<?php echo $mp;?>&kls=<?php echo $kls;?>&ju=<?php echo $ju;?>">
                        ...
                        <div class="d-flex gap-2 ms-auto">
                            ...
                            <a data-toggle="modal" class="btn btn-secondary btn-icon-split" data-target="#createModal">
                             ...
                             <?php
                                if ($barishdr['st_posting'] == 'Y') {
                             ?>
                            <form method="POST" action="list_ujian_redraft_proses.php" onsubmit="return confirm('Kembalikan jadi draft? Jika status menjadi draft ujian ini belum bisa dikerjakan oleh siswa')" style="margin:0">
                            ...
                            ?>
                             ...
                             ?>
                            <form method="POST" action="list_ujian_posting_proses.php" onsubmit="return confirm('Setelah posting maka ujian ini tidak dapat diedit kembali dan sudah dapat dikerjakan oleh siswa')" style="margin:0">
                                <input type='hidden' name='id_ujian_hdr' value='<?php echo $id_ujian_hdr;?>'/>
                                <input type='hidden' name='mp' value='<?php echo $mp;?>'/>
                                <input type='hidden' name='kls' value='<?php echo $kls;?>'/>
                                <input type='hidden' name='ju' value='<?php echo $ju;?>'/>
                            ...
                            <!-- Tombol Hapus -->
                            <form method="POST" action="list_ujian_hapus.php" onsubmit="return confirm('Yakin ingin menghapus draft ujian ini? semua soal di ujian ini juga akan terhapus')" style="margin:0">
                                <input type='hidden' name='id_ujian_hdr' value='<?php echo $id_ujian_hdr;?>'/>
                                <input type='hidden' name='mp' value='<?php echo $mp;?>'/>
                                <input type='hidden' name='kls' value='<?php echo $kls;?>'/>
                                <input type='hidden' name='ju' value='<?php echo $ju;?>'/>
                            ...
                            <?php
                                ...
                                if ($barishdr['st_nonaktif_ujian'] == null AND $barishdr['st_posting'] == 'Y') {
                            ?>
                            <form method="POST" action="list_ujian_nonaktifkan_ujian.php" onsubmit="return confirm('Yakin ingin menonaktifkan ujian ini? Siswa tidak akan bisa mengerjakan ujian walaupun pada hari yang tepat pelaksanaan ujian')" style="margin:0">
                                <input type='hidden' name='id_ujian_hdr' value='<?php echo $id_ujian_hdr;?>'/>
                                <input type='hidden' name='mp' value='<?php echo $mp;?>'/>
                                <input type='hidden' name='kls' value='<?php echo $kls;?>'/>
                                <input type='hidden' name='ju' value='<?php echo $ju;?>'/>
                            ...
                            <?php
                                ...
                                if ($barishdr['st_nonaktif_ujian'] == 'Y' AND $barishdr['st_posting'] == 'Y') {
                            ?>
                            <form method="POST" action="list_ujian_aktifkan_ujian.php" onsubmit="return confirm('Yakin ingin aktifkan kembali ujian ini? Siswa akan bisa mengerjakan ujian pada hari yang tepat pada pelaksanaan ujian')" style="margin:0">
                                <input type='hidden' name='id_ujian_hdr' value='<?php echo $id_ujian_hdr;?>'/>
                                <input type='hidden' name='mp' value='<?php echo $mp;?>'/>
                                <input type='hidden' name='kls' value='<?php echo $kls;?>'/>
                                <input type='hidden' name='ju' value='<?php echo $ju;?>'/>
...
<div class="w-full mx-auto p-4 bg-white shadow rounded-lg mt-6" x-data="soalForm()">
    <form method="post" action="list_ujian_edit_proses.php" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <select required name="kode_mata_pelajaran" id="kode_mata_pelajaran" onchange="loadGuru(); loadUjian()" class="w-full border rounded p-2">
                        ...
                        <?php
                        $sql2 = "SELECT kode_mata_pelajaran, nama_mata_pelajaran 
                        from d_mata_pelajaran order by nama_mata_pelajaran;";
                        $hasil2 = $db->query($sql2);
                        while($baris2 = $hasil2->fetch(PDO::FETCH_ASSOC)) {
                        ?>
                        <option value='<?php echo $baris2['kode_mata_pelajaran']?>' <?php echo ($baris2['nama_mata_pelajaran'] == $barishdr['nama_mata_pelajaran']) ? 'selected' : ''; ?>>
                ...
                <div>
                    ...
                    <select 
                        ...
                        name="uuidguru" 
                        id="uuidguru" 
                        data-selected="<?php echo $barishdr['uuidguru']; ?>" 
                ...
                <div>
                    ...
                    <select 
                        ...
                        name="id_ujian" 
                        id="id_ujian" 
                        data-selected="<?php echo $barishdr['id_ujian']; ?>" 
                        style="width: 100%; padding: 8px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box;">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-3">
                <div>
                    ...
                    <input required name="tanggal_ujian" type="date" class="form-control w-full border rounded p-2" value="<?= $barishdr['tanggal_ujian']; ?>">
                </div>
                <div>
                    <p><b>Durasi Ujian (Menit)</b></p>
                    <input required name="durasi" type="number" class="form-control w-full border rounded p-2" value="<?= $barishdr['durasi']; ?>">
                </div>
                 <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <?php
                    $sqlsoal = "SELECT count(1) jml_soal, ROUND(nilai::numeric, 1) AS nilai
                    from f_soal_dtl
                    where id_ujian_hdr = $id_ujian_hdr
...
function loadUjian() {
    ...
    const selectedId = UjianSelect.getAttribute('data-selected');
        ...
        fetch('get_ujian_by_mapel.php?kode_mata_pelajaran=' + MapelId)
            ...
            .then(data => {
                ...
                data.forEach(item => {
                    ...
                    if (selectedId && selectedId == item.id_ujian) opt.selected = true;
                    UjianSelect.appendChild(opt);
                });
            })
            .catch(err => { UjianSelect.innerHTML = '<option value="">Gagal memuat</option>'; });
    } else {
        UjianSelect.innerHTML = '<option value="">--Ujian--</option>';
    }
}
...
function loadGuru() {
    ...
    const selectedId = GuruSelect.getAttribute('data-selected');
        ...
        fetch('get_guru_by_mapel.php?kode_mata_pelajaran=' + MapelId)
            ...
            .then(data => {
                ...
                data.forEach(item => {
                    ...
                    if (selectedId && selectedId == item.uuidguru) opt.selected = true;
                    GuruSelect.appendChild(opt);
                });
            })
            .catch(err => { GuruSelect.innerHTML = '<option value="">Gagal memuat</option>'; });
    } else {
        GuruSelect.innerHTML = '<option value="">--Guru--</option>';
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    loadGuru();
    loadUjian();
    initEditors(); // init semua editor lama
});
</script>
